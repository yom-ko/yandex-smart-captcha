...
## 1. Resolve instruction conflicts, if any
...
- Prefer the smallest correct local change over architectural change or refactoring.
...
- Do not add speculative error handling solely for impossible or unsupported scenarios.
...
- Never reset, revert, discard, overwrite, or delete existing user work without explicit user approval.
...
- Do not weaken authentication, authorization, sandboxing, branch protection, CI security, dependency security, or validation checks without explicit user approval.
- Do not run destructive commands, force-push, publish packages, deploy, modify production data, or perform irreversible external actions without explicit user approval.
